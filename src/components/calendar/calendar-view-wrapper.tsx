...
export function CalendarViewWithDataFetcher({ initialEvents }: CalendarViewProps) {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            const newEvents = await fetchMonthEvents(start, end);
            setEvents(newEvents);
            setLoading(false);
        };

        // Skip initial fetch if we are on the initial month (optimization? Or just fetch to be safe/consistent)
        // If initialEvents are for "now", and currentDate is "now", we can skip?
        // But navigating back/forth needs fetch.
        ...
        <div className="flex flex-col h-[calc(100vh-10rem)] bg-background border rounded-lg shadow-sm">
            <div className="flex items-center justify-between p-4 border-b">
                <div className="flex items-center gap-2">
                    <h2 className="text-lg font-semibold capitalize w-48">
                        {format(currentDate, 'MMMM yyyy')}
                    </h2>
                    <div className="flex items-center rounded-md border bg-muted/50 p-0.5 ml-4">
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={prevMonth} disabled={loading}>
                            <ChevronLeft className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={nextMonth} disabled={loading}>
                            <ChevronRight className="h-4 w-4" />
                        </Button>
                    </div>
                    <Button variant="outline" size="sm" onClick={today} className="ml-2" disabled={loading}>
                        Today
                    </Button>
